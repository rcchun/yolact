...
class APDataObject:
    ...
    def get_ap50(self):
        ...
        for datum in self.data_points:
            ...
            num_false_negative = self.num_gt_positives - num_true
            precisions.append(precision)
            recalls.append(recall)
            num_true_list.append(num_true)
            num_false_list.append(num_false)
            num_false_negative_list.append(num_false_negative)

        tp = sum(num_true_list)
        fp = sum(num_false_list)
        fn = sum(num_false_negative_list)
        if (tp + fp) == 0:
            pre = 0
        else:
            pre = tp / (tp + fp)
        if (tp + fn) == 0:
            ...
            rec = tp / (tp + fn)
        print('precision : ', pre, 'recall : ', rec, 'tp : ', tp, 'fp :', fp, 'fn : ', fn)
        # Smooth the curve by computing [max(precisions[i:]) for i in range(len(precisions))]
        # Basically, remove any temporary dips from the curve.
        # At least that's what I think, idk. COCOEval did it so I do too.
        for i in range(len(precisions) - 1, 0, -1):
        ...
        # Finally compute the riemann sum to get our integral.
        # avg([precision(x) for x in 0:0.01:1])
        print('AP:', sum(y_range) / len(y_range))
        return sum(y_range) / len(y_range), pre, rec, tp, fp, fn
...
def evaluate_train(tr_wk_id, net: Yolact, dataset, train_mode=False):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                ...
                # plt.yticks([])
                # plt.show()
                # plt.savefig('./Yolact_res101/results/{}.jpg'.format(image_idx), bbox_inches='tight', pad_inches=0,
                #             dpi=300)
                ...
                if not os.path.isdir('./Yolact_res101/results/{}'.format(tr_wk_id)):
                    os.mkdir('./Yolact_res101/results/{}'.format(tr_wk_id))
                plt.imshow(img_numpy)
                plt.title(str(dataset.ids[image_idx]))
                plt.axis('off')
                plt.xticks([])
                plt.yticks([])
                plt.show()
                plt.savefig('./Yolact_res101/results/{}/{}.jpg'.format(tr_wk_id, image_idx), bbox_inches='tight',
            ...
            if args.output_coco_json:
                ...
                if not train_mode:
                    ...
                    with open(args.ap_data_file, 'wb') as f:
                        ...
                        return calc_map50(ap_data, tr_wk_id)
                else:
...
def evaluate(tr_wk_id, ts_wk_id, net:Yolact, dataset, train_mode=False):
    net.detect.use_fast_nms = args.fast_nms
    net.detect.use_cross_class_nms = args.cross_class_nms
    cfg.mask_proto_debug = args.mask_proto_debug
    # TODO Currently we do not support Fast Mask Re-scroing in evalimage, evalimages, and evalvideo
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                ...
                udt = pf_test.update().where(pf_test.c.tr_wk_id == tr_wk_id).values(progress_rate=progress)
                conn = db.connect()
                conn.execute(udt)
                ...
                if res_fp[0]:
                    ins = pf_test.update().where(and_(pf_test.c.ts_wk_id == ts_wk_id,
                                                      pf_test.c.tr_wk_id == tr_wk_id)).values(status=3)
                    conn.execute(ins)
                    break
                if not os.path.isdir('./Yolact_res101/results/{}'.format(tr_wk_id)):
                    os.mkdir('./Yolact_res101/results/{}'.format(tr_wk_id))
...
def calc_map(ap_data):
    ...
    aps50 = [{'box': []} for _ in [50]]
    for _class in range(len(cfg.dataset.class_names)):
        ap_obj50 = ap_data['box'][0][_class]
        if not ap_obj50.is_empty():
            aps50[0]['box'].append(ap_obj50.get_ap50())
    ...
    print(aps[0]['box'][0])
    print(sum(aps[0]['box']))
    print('mAP : ', sum(aps[0]['box']) / len(aps[0]['box']))
    tp_sum = aps50[0]['box'][0][3]+aps50[0]['box'][1][3]\
             +aps50[0]['box'][2][3]+aps50[0]['box'][3][3]+aps50[0]['box'][4][3]
    fp_sum = aps50[0]['box'][0][4]+aps50[0]['box'][1][4]\
             +aps50[0]['box'][2][4]+aps50[0]['box'][3][4]+aps50[0]['box'][4][4]
    fn_sum = aps50[0]['box'][0][5]+aps50[0]['box'][1][5]\
             +aps50[0]['box'][2][5]+aps50[0]['box'][3][5]+aps50[0]['box'][4][5]
    print('tp_sum : ', tp_sum, 'fp_sum : ', fp_sum, 'fn_sum : ', fn_sum)
    print('Recall : ', tp_sum/(tp_sum+fn_sum), 'Precision : ', tp_sum/(tp_sum+fp_sum))
...
def calc_map50(ap_data, tr_wk_id):
    print('Calculating mAP...')
    aps = [{'box': [], 'mask': []} for _ in iou_thresholds]
    aps50 = [{'box': []} for _ in [50]]
    for _class in range(len(cfg.dataset.class_names)):
    ...
    test_metric = {}
    test_metric_per_cat = {}
    for idx, class_ in enumerate(cfg.dataset.class_names):
        test_metric_per_cat.update({"{}".format(class_): {'TP': aps50[0]['box'][idx][3],
                                                                   'FP': aps50[0]['box'][idx][4],
                                                                   'FN': aps50[0]['box'][idx][5],
                                                                   'Recall': aps50[0]['box'][idx][2],
                                                                   'Precision': aps50[0]['box'][idx][1],
                                                                   'AP': aps50[0]['box'][idx][0]}})
    test_metric.update({"CLASS_RESULT": test_metric_per_cat})
    test_metric.update({"AVERAGE_RESULT": {'TP': tp_sum,
                                           'FP': fp_sum,
                                           'FN': fn_sum,
                                           'Recall': tp_sum / (tp_sum + fn_sum),
                                           'Precision': tp_sum / (tp_sum + fp_sum),
                                           'mAP': sum(aps[0]['box']) / len(aps[0]['box'])}})
    pf_test = metadata.tables['pf_test']
    udt = pf_test.update().where(pf_test.c.tr_wk_id == tr_wk_id).values(result=test_metric)
    conn = db.connect()
    conn.execute(udt)
...
def main(ts_wk_id, tr_wk_id, dats_id, threshold, resolution):
    parse_args()

    if args.config is not None:
        set_cfg(args.config)

    pf_dataset = metadata.tables['pf_dataset']
    stmt_dir = select([pf_dataset.c.data_directory]).where(pf_dataset.c.dats_id == dats_id)
    conn = db.connect()
    res_dir = conn.execute(stmt_dir).fetchone()
    data_directory = res_dir[0] + '/'
    json_file = data_directory + "via_export_json.json"
    with open(json_file) as f:
        imgs_anns_class = json.load(f)
        classification = []

        for idx, v in enumerate(imgs_anns_class.values()):
            annos_class = v["regions"]
            for i in range(len(annos_class)):
                class_ = annos_class[i]["region_attributes"]
                if {'class_num': class_['classification'], 'class_name': class_['class_name']} not in classification:
                    classification.append({'class_num': class_['classification'], 'class_name': class_['class_name']})
    res = sorted(classification, key=(lambda x: x['class_num']))
    NUM_CLASSES = len(res)
    cfg.num_classes = NUM_CLASSES + 1
    classification_list = []
    for i in range(len(res)):
        classification_list.append(res[i]['class_name'])
    print('class_list : {}'.format(classification_list))
    pf_train = metadata.tables['pf_train']
    stmt_weight = select([pf_train.c.weight_path]).where(pf_train.c.tr_wk_id == tr_wk_id)
    res_wei = conn.execute(stmt_weight).fetchone()
    print(res_wei[0])
    args.trained_model = res_wei[0]
    ...
    with torch.no_grad():
        ...
        cfg.dataset.has_gt = tuple(classification_list)
        ...
        evaluate(tr_wk_id, ts_wk_id, net, dataset)
